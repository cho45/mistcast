...
export class AudioStreamProcessor extends AudioWorkletProcessor {
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    // 入力サンプルがあればWorkerへ送信 (簡易実装: transferなしでそのまま送るか、RecycleTransferを使うか)
    // ここではシンプルにそのまま postMessage する
    if (inL && this.inputPort) {
        this.inputPort.postMessage({ type: "input", data: inL });
    }
