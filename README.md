...
| **チップレート** | 8,000 cps |
| **キャリア周波数** | 15,000 Hz |
| **実効速度 (Goodput)** | **~ 1,000 bps** (3 pkts/frame) |
| **マルチパス対策** | **FDE + 自動パス選択** |
...
- **データサイズ制限**: `lt_k` が 8ビット（最大255）であるため、一度に送信可能な最大データサイズは **$255 \times 16 = 4,080$ バイト**までに抑えられています。これは「小さなデータのブロードキャスト」という本プロジェクトの設計意図に基づいています。本実装では、スループットと受信完了までの待機時間のバランス、および**CRC16の衝突確率**（後述）の観点から、**1KB 程度のデータ伝送**を現実的な運用範囲として想定しています。
...
`K=16/32` は低SNRでの取りこぼし改善は見込める一方、一連の通信におけるトータルのパケット数が増えるほど、一度でも誤受理が発生する累積リスクが無視できない確率となります。このため、信頼性を確保する観点からは最大パケット数は少ない（＝データサイズが小さい）ほど望ましく、255パケットフルに使用するような運用は非推奨です。
