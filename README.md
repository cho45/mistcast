...
- **固定パターン**: `0xA5C3_E6E2` (下位ビットから使用)
  - 理由: DBPSK差動後の位相系列において、下位8bit (`0xE2`) / 下位16bit (`0xE6E2`) の自己相関サイドローブが小さいため。
...
- **FEC (CRC-aided)**: 畳み込み符号（拘束長K=7, R=1/2）と **List Viterbi** 復号。
  - 最もらしいパスを単一出力するのではなく、複数の候補（例: K=32）を出し、CRC-24が通ったものを正解とする手法を採用しています。
...
| 必要パケット数 ($N$) | データサイズ | $K=8$ | $K=16$ | $K=32$ |
| :--- | :--- | :--- | :--- | :--- |
...
### クイックスタート
1. **ビルドツール導入**: `make toolchain-install`
2. **依存関係のインストール**: `npm install`
3. **WASM コアのビルド**: `npm run build:wasm`
4. **開発サーバーの起動**: `npm run dev` (`http://localhost:5173`)

### ビルドツールチェーン

WASM ビルドで必要な Rust ツールは `Makefile` で固定化しています。

- 導入: `make toolchain-install`
- 確認: `make toolchain-check`

`toolchain-install` は以下を導入します。

- rust target: `wasm32-unknown-unknown`
- `wasm-pack` `0.14.0`

通常は `wasm-bindgen-cli` の手動インストールは不要です（`wasm-pack` が必要に応じて扱います）。
ただし、オフライン環境や制限付き環境では別途セットアップが必要になる場合があります。

バージョンを一時的に切り替えたい場合は、`make` 変数で上書きできます。

```bash
make toolchain-install WASM_PACK_VERSION=0.14.0
```
...
`toolchain-check` で最低限確認したい項目:

- `wasm-pack` が表示されること
- `wasm32-unknown-unknown: installed` が表示されること

確認後、WASM ビルドとテストを実行します。
...
#### native テスト時の `RUST_TEST_THREADS=12` 推奨理由

`dsp` クレートの native テストでは、環境によって `cargo test` のデフォルト並列数（例: 24 スレッド）でまれに `SIGFPE`（floating point exception）が発生することがあります。

- **観測された現象**:
  - `cargo test`（デフォルト並列）でのみ `SIGFPE` が発生
  - `-- --test-threads=1,2,4,8` では安定して通過
  - クラッシュ地点は `sin/cos` 呼び出し経路（glibc 側）で観測
- **補足**:
  - テストコード側に明示的な共有状態（static/global）は見当たらない
  - 根本原因（どこで FPU 状態が変わるか）は未確定
  - 実装側に回避ロジックを入れるとコードが複雑化するため、現時点では運用で回避

そのため、native でのテストは以下を推奨します。

```bash
cd dsp
RUST_TEST_THREADS=12 cargo test --release
```
...
$ cargo run --release --bin dsss_e2e_eval -- \
        ...
        --mode sweep-awgn \
        --packets-per-frame 2 \
        --sweep-awgn '0.5,0.7,0.8,0.9,1.0,1.1,1.2' \
...
| :----------------------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- |
| sweep_awgn(sigma=0.500)        | 14.3483         | 1.0000          | 1.0000          | 1.0000          | 5.51e-5         | 1061.4156       | 937.8757        |
| sweep_awgn(sigma=0.700)        | 11.4257         | 1.0000          | 1.0000          | 1.0000          | 0.0031          | 1061.4156       | 937.8757        |
| sweep_awgn(sigma=0.800)        | 10.2659         | 0.9880          | 1.0000          | 0.9880          | 0.0120          | 1048.6276       | 929.3496        |
| sweep_awgn(sigma=0.900)        | 9.2428          | 0.9608          | 1.0000          | 0.9637          | 0.0344          | 1019.8542       | 903.7712        |
| sweep_awgn(sigma=1.000)        | 8.3277          | 0.8645          | 1.0000          | 0.8697          | 0.0642          | 917.5491        | 814.8926        |
| sweep_awgn(sigma=1.100)        | 7.4998          | 0.5181          | 1.0000          | 0.5292          | 0.1058          | 549.8901        | 491.9169        |
| sweep_awgn(sigma=1.200)        | 6.7440          | 0.1416          | 0.9940          | 0.1460          | 0.1459          | 150.2607        | 136.9684        |
