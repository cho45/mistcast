...
- **FEC (CRC-aided)**: 畳み込み符号（拘束長K=7, R=1/2）と **List Viterbi** 復号。
  - 最もらしいパスを単一出力するのではなく、複数の候補（例: K=8）を出し、CRC-16が通ったものを正解とする手法を採用しています。
- **CRC-16**: パケット単位の誤り検出。ペイロードサイズに最適化された多項式（poly=`0x5935`）を使用しています。
...
#### 仕様制約: CRC16 と List Viterbi による誤受理リスクの累積
...
##### 1. CRC16 の衝突確率とパケット数
CRC16 は 16bit の検査値を持つため、ランダムなデータに対して誤って「正解」と判定してしまう確率は理論上 $1/65536$ ($≈ 1.5 \times 10^{-5}$) です。一連のデータ復号に必要なソースブロック数を $N$ とすると、少なくとも1つのパケットで誤受理が発生する累積確率 $P_{any}$ は以下のようになります（近似）。

$$P_{any}(N) \approx 1 - (1 - 1/65536)^N$$

##### 2. List Viterbi によるリスクの増幅
M-ARY モードでは、低SNR環境での感度を向上させるために **List Viterbi (top-K)** を採用しています。これはViterbi復号の候補を $K$ 個生成し、順にCRC16を評価する手法ですが、これは1パケットにつき実質的に CRC16 の評価を $K$ 回試行することと同等です。これにより、1パケットあたりの誤受理確率は $K/65536$ へと増幅されます。
...
| :--- | :--- | :--- | :--- | :--- |
| **64 pkts** | **~1.0 KB** | **0.8%** | 1.5% | 3.1% |
| **128 pkts** | ~2.0 KB | 1.5% | 3.1% | 6.1% |
| **255 pkts** | ~4.0 KB | 3.1% | 6.0% | 11.7% |

##### 結論
- **データサイズは小さいほど安全**: パケット数が増えるほど累積リスクが高まるため、実用上は **1KB 程度** までの運用を推奨しています。
- **K値のトレードオフ**: 感度改善と誤受理リスクのバランスから、デフォルトでは $K=8$ を採用しています。$K=32$ などの高設定は、特に大きなデータを送る際、無視できない確率で復号失敗を引き起こす可能性があります。
