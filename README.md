...
| **チップレート** | 8,000 cps |
| **キャリア周波数** | 15,000 Hz |
| **実効速度 (Goodput)** | **~ 1,100 bps** (3 pkts/frame) |
| **マルチパス対策** | **FDE + 自動パス選択** |
...
- **CRC-24**: パケット単位の誤り検出。OpenPGP 互換パラメータ（poly=`0x864CFB`, init=`0xB704CE`）を使用しています。
...
Mistcastは、**「頻繁に同期（プリアンブル）を送り直す」**設計を採用しています。デフォルト（M-ARY, 3 pkts/frame）では、約 526ms ごとに同期信号が挿入されます。
...
同期の堅牢性（やり直しが効く頻度）を維持するため、同期周期を **約 526ms** に設定した場合の構成です。

| 項目 | 値 |
| :--- | :--- |
| **フレーム構成** | 1 プリアンブル + 1 Sync Word + 3 パケット |
| **同期周期** | **525.75 ms** |
| **伝送量 / フレーム** | **576 bits** (データ本体) |
| **実効速度 (Goodput)** | **~ 1,100 bps** |
| **同期オーバーヘッド** | **6.4%** |
...
| :----------------------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- | :-------------- |
| sweep_awgn(sigma=0.000)        | -               | 1.0000          | 1.0000          | 1.0000          | 0.0000          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.100)        | 28.3279         | 1.0000          | 1.0000          | 1.0000          | 0.0000          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.200)        | 22.3073         | 1.0000          | 1.0000          | 1.0000          | 0.0000          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.300)        | 18.7855         | 1.0000          | 1.0000          | 1.0000          | 0.0000          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.400)        | 16.2867         | 1.0000          | 1.0000          | 1.0000          | 0.0000          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.500)        | 14.3485         | 1.0000          | 1.0000          | 1.0000          | 1.83e-4         | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.700)        | 11.4260         | 1.0000          | 1.0000          | 1.0000          | 0.0042          | 1095.4875       | 973.7667        |
| sweep_awgn(sigma=0.800)        | 10.2661         | 0.9739          | 1.0000          | 0.9739          | 0.0148          | 1066.9095       | 948.3641        |
| sweep_awgn(sigma=0.900)        | 9.2431          | 0.9014          | 1.0000          | 0.9067          | 0.0364          | 987.5264        | 872.2290        |
