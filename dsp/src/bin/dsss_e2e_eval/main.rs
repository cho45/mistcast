...
define_metrics! {
    ...
    // 理論系: 実信号AWGNの分散比 SNR から C/N0・Eb/N0 を出すため、B=fs/2 (Nyquist帯域) を使う。
    ebn0_db,                       "AWGN 仮定 (B=sample_rate/2) から換算した理論 Eb/N0 [dB]",       |ctx, m| m.awgn_ebn0_db(ctx.sigma, ctx.sample_rate, ctx.bit_rate).into(), default;
    cn0_db,                        "AWGN 仮定 (B=sample_rate/2) から換算した理論 C/N0 [dB-Hz]",      |ctx, m| m.awgn_cn0_db(ctx.sigma, ctx.sample_rate).into(), default;
    snr_wideband_db,               "AWGN 分散比 SNR [dB] (定義帯域: B=sample_rate/2)", |ctx, m| m.awgn_snr_db(ctx.sigma).into(), default;
    ...
    accepted_packet_count,         "受理パケット総数",                               |_, m| m.total_accepted_packets.into();
    false_accept_count,            "false accept パケット数",                        |_, m| m.total_false_accepted_packets.into();
    ...
    avg_last_est_snr_db,           "受信器内部の推定 SNR の平均 [dB] (定義帯域: B≈chip_rate)", |_, m| m.avg_last_est_snr_db().into(), default;
...
mod tests {
    ...
    fn test_cn0_db_formula_and_consistency() {
        ...
        let expected_bandwidth = sample_rate * 0.5;
        let expected_cn0 = snr_db + 10.0 * expected_bandwidth.log10();
        assert!((cn0_db - expected_cn0).abs() < 1e-4);

        let expected_ebn0 = snr_db + 10.0 * (expected_bandwidth / bit_rate).log10();
