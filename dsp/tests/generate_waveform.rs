...
fn test_mary_mode_generates_valid_wav() {
    ...
    assert_eq!(
        u32::from_le_bytes([contents[24], contents[25], contents[26], contents[27]]),
        48000,
        "Invalid sample rate"
    );
    ...
    // チャンネル数を確認 (mono = 1)
    assert_eq!(
        u16::from_le_bytes([contents[22], contents[23]]),
        1,
    ...
    assert_eq!(
        u16::from_le_bytes([contents[34], contents[35]]),
        16,
    ...
    assert_eq!(
        data_size + 44,
        contents.len(),
        "Data size mismatch: data_size={}, file_size={}",
        data_size,
        contents.len()
    );
    ...
    assert!(
        stdout.contains("フレーム数:"),
        "Output should contain frame count"
    );
}
...
fn test_dsss_mode_generates_valid_wav() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("モード: Dsss"),
        "Output should show DSSS mode"
    );
}
...
fn test_default_text_generates_valid_output() {
    ...
    assert!(
        ...
        "Output should contain default text"
    );
...
fn test_duration_is_approximate_ends_on_frame_boundary() {
    ...
    assert!(
        stdout.contains("目安の長さ: 3.7 秒"),
        "Should show target duration"
    );
    assert!(
        stdout.contains("実際の長さ:"),
...
fn test_custom_sample_rate() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("サンプルレート: 44100 Hz"),
        "Should show 44.1kHz"
    );
}
...
fn test_unicode_text() {
    ...
    assert!(
        stdout.contains("17 バイト"),
        "Should show correct byte count with header"
    );
...
fn test_size_prefix_format() {
    ...
    assert_eq!(
        data_with_header[1], 0x03,
        "Second byte should be 0x03 (size of 'ABC')"
    );
    assert_eq!(
        &data_with_header[2..],
        b"ABC",
    ...
    assert_eq!(
        large_size[0], 0x01,
        "High byte should be 0x01 for 300 bytes"
    );
    assert_eq!(
        large_size[1], 0x2C,
        "Low byte should be 0x2C (300 = 0x012C)"
    );
}
